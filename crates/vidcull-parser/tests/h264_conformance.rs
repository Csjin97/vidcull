...
fn decode_native(h264: &[u8]) -> LumaFrame {
    ...
    decode_intra_frame(&sps, &pps, &slices).expect("native decode")
}
...
fn assert_bit_exact(name: &str, frame: &LumaFrame, reference: &[u8]) {
        ...
        .find(|(_, (a, b))| a != b)
    {
        ...
        let diffs = frame
            ...
            .count();
        panic!(
            "[{name}] native decode differs from ffmpeg: {diffs}/{} samples; \
...
#[test]
fn native_cavlc_decode_is_bit_exact_vs_committed_reference() {
...
fn committed_references_match_ffmpeg() {
    ...
    for &name in FIXTURES {
        ...
        let out = tmp.join(format!("vidcull_conf_{name}.y8"));
        ...
        assert_eq!(
            committed, fresh,
            "[{name}] committed .y8 is stale vs ffmpeg — regenerate with scripts/gen-h264-conformance.ps1"
        );
    }
}

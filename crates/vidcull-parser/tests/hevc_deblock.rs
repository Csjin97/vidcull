...
const FIXTURES: &[(&[u8], &[u8], usize, usize)] = &[
    (
        include_bytes!("fixtures/hevc-deblock/clip.hevc"),
        include_bytes!("fixtures/hevc-deblock/clip.y8"),
        160,
        90,
    ),
    (
        include_bytes!("fixtures/hevc-deblock/clip2.hevc"),
        include_bytes!("fixtures/hevc-deblock/clip2.y8"),
        256,
        144,
    ),
];
...
#[test]
fn deblock_matches_ffmpeg_luma() {
    for &(clip, golden, w, h) in FIXTURES {
        deblock_one(clip, golden, w, h);
    }
}

fn deblock_one(clip: &[u8], golden: &[u8], width: usize, height: usize) {
...
fn fixtures_are_present() {
    let dir = Path::new(env!("CARGO_MANIFEST_DIR")).join("tests/fixtures/hevc-deblock");

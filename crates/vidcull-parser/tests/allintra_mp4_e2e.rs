...
const FIXTURES: &[(&str, usize, usize)] = &[("clip_h264", 160, 90), ("clip_h265", 256, 144)];
...
fn fixture_dir() -> PathBuf {
    Path::new(env!("CARGO_MANIFEST_DIR"))
        .join("tests")
        .join("fixtures")
        .join("allintra-mp4-e2e")
}

#[test]
fn native_path_is_taken_for_no_stss_allintra_and_phash_matches_ffmpeg_gray() {
    ...
    for &(name, width, height) in FIXTURES {
        ...
        let label = format!("{name} mp4");

        let decoded = probe_and_decode_sparse(&bins, &mp4, 4)
            .unwrap_or_else(|e| panic!("[{label}] probe_and_decode_sparse: {e:?}"));

        assert_eq!(
            decoded.decode_path,
            DecodePath::Native,
            "[{label}] all-intra (no-stss) MP4 must decode on the native path, not the ffmpeg fallback"
        );
